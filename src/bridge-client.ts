...
async function request(path: string, init: RequestInit, timeoutMs: number): Promise<any> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 503) throw new BulcNotRunningError(body || res.statusText);
...
export async function fetchTools(): Promise<BridgeTool[]> {
...
export async function callTool(name: string, args: unknown): Promise<{ message: string; action?: any }> {
...
export async function startRun(folderName?: string, solverType?: string): Promise<any> {
  return post("/run", { folder_name: folderName, solver_type: solverType });
}

export async function health(): Promise<{ ok: boolean; hasProject?: boolean; toolCount?: number } | null> {
  try {
